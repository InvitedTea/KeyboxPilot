import { ArrowDownTrayIcon, ClipboardIcon, CubeTransparentIcon } from "@heroicons/react/20/solid";
...
export default function AppPreviewContract() {
  ...
  const analyzeContract = async () => {
    const response = await fetch('/api/analyze-contract', {
      ...
      body: JSON.stringify({
        contractData: contract, // or the specific part of the contract you want to send
      }),
    });
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-start md:flex-row">
        ...
        <div>
          ...
          >
            <span className="hidden md:block">Help Me Analyze this Smart Contract</span>
          </AppButton>
        </div>
      </div>
